...
class XbotArmTaskBase(XbotArmBase):
    target_control = np.zeros(6)  # 6关节目标控制
    ...
    def random_table_texture(self):
        try:
            ...
            print("Warning: table texture not found")
    ...
    def domain_randomization(self):
        # 默认实现，子类可以重写以添加特定的领域随机化
        pass

    def checkActionDone(self):
        # 检查关节是否到达目标位置并静止
        joint_done = np.allclose(self.sensor_joint_qpos, self.target_control, atol=3e-2) and np.abs(self.sensor_joint_qvel).sum() < 0.1
        self.delay_cnt -= 1
    ...
    def on_key(self, window, key, scancode, action, mods):
        ret = super().on_key(window, key, scancode, action, mods) if hasattr(super(), 'on_key') else None
